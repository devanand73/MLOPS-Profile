...
### Scalable Data Pipeline: Serverless ETL to Reporting on AWS

In the fast-evolving landscape of data science, efficient data management systems are crucial for the dynamic needs of businesses. A serverless data pipeline offers a potent solution, providing scalability and cost-effectiveness
...
This project exemplifies the entire lifecycle of a machine learning project incorporating a custom preprocessing pipeline for data normalization and feature extraction. The trained model is serialized and integrated into a user-friendly Streamlit web application deployed on Hugging Face. This project highlights the practical application of machine learning to real-world problems and the value of making predictive tools easily available to users.

[View code on Colab](https://colab.research.google.com/drive/1PAlDqN4mhd9c4YzZmfMUVKUdnFIpxr1r?usp=sharing)

[Try! predict Wine Quality](https://huggingface.co/spaces/anand04051973/predict-wine-quality)
...
### RenalNet: Advanced Kidney Disease Detection with MLflow & DVC, FastAPI, Docker, AWS, and GitHub CI/CD"
This project serves as a comprehensive example, providing all the necessary code to deploy a model through an API. It utilizes MLflow for experiment tracking and DVC for data management, automating the workflow with a CI/CD pipeline via GitHub Actions. The solution is containerized with Docker, ensuring an isolated and consistent environment, and employs FastAPI for efficient handling of API requests, demonstrating end-to-end functionality from model training to serving predictions. Deployed on AWS to ensure availability and scalability, this combination of MLOps tools and cloud deployment offers a robust, efficient, and user-friendly solution for medical diagnostics. It highlights the project's dedication to leveraging data science for healthcare advancements.
