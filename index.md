...
### Deploy ML model using Docker & FastAPI for inference
This project serves as a complete example, encompassing all the required code to serve a model through an API. I have meticulously prepared the full suite of scripts needed for the deployment and inference of a machine-learning model trained on the Iris dataset. The solution is containerized using Docker, ensuring an isolated and consistent environment and employs FastAPI to handle API requests effectively, showcasing end-to-end functionality from model training to serving predictions.
...
[View code on Colab](https://colab.research.google.com/drive/13_ha3KWk1z7tTvEyh4Ug3pEQITp7jlJq?usp=sharing)
